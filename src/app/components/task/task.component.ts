...
export class TaskComponent implements OnInit {
  ...
  toggleReminder (task: Task) {
    if (this.service.updateTask(task)) task.isCompleted = !task.isCompleted
    if (task.isCompleted) alert(task.text + ' is registered as completed')
    else alert(task.text + ' is registered as incompleted')
  }
}
